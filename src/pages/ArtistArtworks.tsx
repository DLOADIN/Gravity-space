...
export default function ArtistArtworks() {
  ...
  const [editingArtwork, setEditingArtwork] = useState<ArtistArtwork | null>(null);
  ...
  const fetchArtistArtworks = async () => {
    try {
      const data = await api.get<{ artworks: ArtistArtwork[] }>(API_ENDPOINTS.artworks + '/my-artworks');
      setArtworks(data.artworks);
    } catch (error) {
      toast({
        title: 'Error',
        description: 'Failed to fetch your artworks',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ title: '', description: '', price: '', image_url: '', category_id: '', status: 'available' });
  ...
  const handleEdit = (artwork: ArtistArtwork) => {
